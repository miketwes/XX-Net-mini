...
ST                     = Internet
L                      = Cernet
O                      = GoAgent-XX-Net-mini-4.5.3
OU                     = GoAgent-XX-Net-mini-4.5.3 Root
CN                     = GoAgent-XX-Net-mini-4.5.3 CA
...
def get_cert(commonname, sans=None, full_name=False):
    ...
    sr = '0x%s' % hashlib.md5((str(commonname) + str(datetime.datetime.now())).encode("utf-8")).hexdigest()
    ...
    with ca_lock:
        ...
        if os.path.exists(certfile):
            ...
            if(platform.system()=='Windows'):
                ...
                ST                     = Internet
                L                      = Cernet
                O                      = GoAgent-XX-Net-mini-4.5.2
                OU                     = GoAgent-XX-Net-mini-4.5.2 Root
                CN                     = GoAgent-XX-Net-mini-4.5.2 CA
                emailAddress           = XX-Net@gmail.com
                [ v3_req ]
                basicConstraints = CA:FALSE
                keyUsage = nonRepudiation, digitalSignature, keyEncipherment
                ...
                '''
                script1 =  script1 % {'commonname': commonname}
                script2="openssl req -new -key " + cert_keyfile + '''   -config <(echo "$CONFIG")'''
                script= script1+script2
                p = subprocess.Popen(['bash', '-c', script], stdout=subprocess.PIPE , stderr=subprocess.PIPE)
                stdout, stderr = p.communicate()
                os.environ['request']=stdout.decode()
                script3='''openssl x509 -req -days  3650 -in  <(echo "$request")  -CA ''' + ca_cert + ''' -CAkey ''' + ca_key + ''' -set_serial ''' + sr + ''' -extensions v3_req -extfile  <(echo "$CONFIG")'''
                script4= script1+script3
                p = subprocess.Popen(['bash', '-c', script4], stdout=subprocess.PIPE , stderr=subprocess.PIPE)
                stdout, stderr = p.communicate()
